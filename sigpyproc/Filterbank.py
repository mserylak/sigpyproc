...
class Filterbank(object):
    ...
    def invertFreq(self,gulp=512,start=0,nsamps=None,filename=None,back_compatible=True):
        ...
        """
        if filename is None:
            filename = "{0}_inverted.fil".format(self.header.basename)
    ...
    def upTo8bit(self,filename=None,gulp=512,back_compatible=True):
        ...
        """
        if filename is None:
            filename = "{0}_8bit.fil".format(self.header.basename)
    ...
    def applyChannelMask(self,chanmask,outfilename=None,gulp=512,back_compatible=True):
        ...
        if outfilename is None:
            outfilename = "{0}_masked.fil".format(self.header.basename)
    ...
    def downsample(self,tfactor=1,ffactor=1,gulp=512,filename=None,back_compatible=True):
        ...
        """
        if filename is None:
            filename = "{0}_f{1}_t{2}.fil".format(self.header.basename,ffactor,tfactor)
    ...
    def split(self,start,nsamps,filename=None,gulp=1024,back_compatible=True):
        ...
        if filename is None:
            ...
            filename = "{0}_{1}_{2}.fil".format(self.header.basename,start,start+nsamps)
    ...
    def splitToChans(self,gulp=1024,back_compatible=True):
        ...
        tim_ar_c = as_c(tim_ar)
        out_files = [self.header.prepOutfile("{0}_chan{1:04d}.tim".format(self.header.basename,ii),
...
class FilterbankBlock(np.ndarray):
    ...
    def toFile(self,filename=None,back_compatible=True):
        ...
        """
        if filename is None:
            filename = "{0}_{1}_to_{2}.fil".format(self.header.basename,self.header.tstart,
